...
#     help = "Import teacher activity data from Excel"

#     def handle(self, *args, **kwargs):

#         file_path = "Savra_Teacher Data Set.xlsx"
...
#             if pd.isna(row["Teacher_id"]) or pd.isna(row["Teacher_name"]) or pd.isna(row["Created_at"]):
#                 skipped_count += 1
#                 continue
...
#                     teacher=teacher,
#                     activity_type=str(row["Activity_type"]).lower().strip(),
#                     created_at=row["Created_at"],
#                     subject=str(row["Subject"]).strip().title(),   # normalization
#                     class_name=str(row["Grade"]).strip()
#                 )
#                 created_count += 1
...
#                 self.stdout.write(self.style.WARNING(f"Row skipped due to error: {e}"))
#                 skipped_count += 1
